...
class MainActivity : AppCompatActivity() {
    ...
    private inner class LifetimeServiceHandler(lifeServiceLooper: Looper):
        Handler(lifeServiceLooper) {
    ...
    /*private val receiveLifetimeBr: BroadcastReceiver by lazy {
        object: BroadcastReceiver() {
            override fun onReceive(p0: Context?, intent: Intent?) {
                intent?.getIntExtra(LifetimeStartedService.EXTRA_LIFETIME, 0).also {
                    lifetime -> activityMainBinding.serviceLifetimeTv.text = lifetime.toString()
                }
            }
        }
    }*/
